...
Ein Objekt ist eine einzelne Einheit, die Sie in docupike dokumentieren.
...
!!! info "Objekt oder Attribut?"
    ...
    In dem mit docupike gelieferten vordefinierten Satz ist eine Festplatte ein Attribut. Wenn Sie jedoch der Meinung sind, dass es für Sie sehr wichtig ist, alle Ihre Festplatten im Detail zu verfolgen, können Sie eine eigene Objektklasse für Festplatten erstellen.
...
Objekte können im Hauptbildschirm des `Finders` erstellt werden. Wählen Sie auf der linken Seite eine Objektklasse aus (z. B. `Server`) und klicken Sie oben in der Liste auf die Schaltfläche `Neues Objekt +`. Es öffnet sich ein Dialogfeld, in dem Sie einige zusätzliche Optionen für die Objekterstellung festlegen können, z. B. das gleichzeitige Erstellen mehrerer Objekte und die Nummerierung.
...
Objekte haben verschiedene Status, die den Zustand ihrer Existenz in der Dokumentation definieren. Die Status sind "aktiv", "archiviert" oder "gelöscht". Ein typischer Lebenszyklus könnte wie folgt aussehen:
