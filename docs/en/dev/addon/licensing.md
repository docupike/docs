...
integrate into i-doit up's ecosystem. That said one can say that add-ons are encapsulated, responsible and
self-sufficient software. The API-based communication is the only way i-doit up and add-ons can communicate
and exchange data.

Because of this perspective i-doit up shifts as much responsibility to add-ons as possible to transfer control about
add-on related aspects.
Licensing is also considered as add-on related why the decision maker should be the add-on itself.

## Technical workflow

i-doit up licenses allow to include add-on specific information and should therefore serve as main resource in
the decision process. This is the preferred way.
To expose the license to registered add-ons i-doit up will embedded these information into each request sent to the
add-on.
...
Another case is the add-on registration procedure which is the first interaction between add-on and i-doit up.
i-doit up's add-on infrastructure allows add-ons to decide whether a registration is possible or not.
